...
public class SinkRenderer implements BlockEntityRenderer<SinkCabinetBlockEntity> {
		...
		if (item != null && !item.isEmpty()) {
			if (item.getItem() instanceof BlockItem b) {
				...
				this.render(pPoseStack, pBufferSource, texture -> RenderType.entityTranslucent(texture), pPackedLight,
						pPackedOverlay, pPartialTick, new BakedModelRenderable.Context(ModelData.EMPTY), pBlockEntity.getBlockPos(),
						copiedQuads);
	...
	public void render(PoseStack poseStack, MultiBufferSource bufferSource,
					   ...
					   BakedModelRenderable.Context context, BlockPos pos, List<BakedQuad> copiedQuads) {
...
//			poseStack.translate(0, -1.05, 0);
		...
		if (copiedQuads != null && !copiedQuads.isEmpty()) {
			...
			if (copiedQuads.size() > 2 && copiedQuads.get(2) != null)
				uvEast = RenderUtil.getUVFromSprite(copiedQuads.get(2).getSprite(), 2, 2, 12, 5);
			...
			RenderUtil.buildInvertedCubePillar(new Vector3f(1.015f / 16f, 12f / 16f, 1.015F / 16f),
					...
					RenderUtil.WHITE, lightmap, uvEast, null, uvBottom, OverlayTexture.NO_OVERLAY, poseStack);
...
//				uvTop = RenderUtil.getuvFromSprite(copiedQuads.get(0).getSprite(), 0, 0, 0, 0);
			...
			RenderUtil.buildCube(new Vector3f(0, 14f / 16f, 14f / 16f), new Vector3f(1, 4f / 16f, 2f / 16f),
					...
					poseStack, null, uvBottom, uvNorth, uvSouth, uvEast, uvWest);
...
//				uvTop = RenderUtil.getuvFromSprite(copiedQuads.get(0).getSprite(), 0, 0, 8, 2);
			...
			RenderUtil.buildCube(new Vector3f(4f / 16f, 18f / 16f, 14f / 16f),
					...
					lightmap, OverlayTexture.NO_OVERLAY, poseStack, null, uvBottom, uvNorth, uvSouth,
					uvEast, uvWest);
			...
			RenderUtil.buildCube(new Vector3f(0, 14f / 16f, 0), new Vector3f(1.02f / 16f, 2f / 16f, 14F / 16F),
					vertexconsumer, matrix4f, matrix3f, RenderUtil.WHITE, lightmap, OverlayTexture.NO_OVERLAY,
					poseStack, null, uvBottom, uvNorth, null, null, uvWest);
			...
			RenderUtil.buildCube(new Vector3f(14.98f / 16f, 14f / 16f, 0), new Vector3f(1.01f / 16f, 2f / 16f, 14F / 16F),
					vertexconsumer, matrix4f, matrix3f, RenderUtil.WHITE, lightmap, OverlayTexture.NO_OVERLAY,
					poseStack, null, uvBottom, uvNorth, null, uvEast, null);
			...
			RenderUtil.buildCube(new Vector3f(1f / 16f, 14f / 16f, 0), new Vector3f(14f / 16f, 2f / 16f, 1.042F / 16F),
					vertexconsumer, matrix4f, matrix3f, RenderUtil.WHITE, lightmap, OverlayTexture.NO_OVERLAY,
					poseStack, null, uvBottom, uvNorth, null, null, null);
	...
	private void drawLip(BlockPos pos, float h, float w, float l, int lightmap, VertexConsumer vertexconsumer,
		...
		if (copiedQuads.get(0) != null) {
			...
			vertexconsumer.vertex(matrix4f, -1, 0, 0).color(255, 255, 255, 255).uv(u0, v0 + vUnit * 2)
					...
					.normal(Direction.DOWN.getNormal().getX(), Direction.DOWN.getNormal().getY(),
			...
			vertexconsumer.vertex(matrix4f, -1 + 2f / 16f, 4f / 16f, 0).color(255, 255, 255, 255).uv(u0, v0)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap).normal(Direction.UP.getNormal().getX(),
			...
			vertexconsumer.vertex(matrix4f, -14f / 16f, 0, 1).color(255, 255, 255, 255).uv(u0, v1)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.NORTH.getNormal().getX(), Direction.NORTH.getNormal().getY(),
			...
			vertexconsumer.vertex(matrix4f, -14f / 16f, 4f / 16f, 0).color(255, 255, 255, 255)
					.uv(u0 + uUnit * 4, v0).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.NORTH.getNormal().getX(), Direction.NORTH.getNormal().getY(),
							Direction.NORTH.getNormal().getZ());

			vertexconsumer.vertex(matrix4f, -14f / 16f, 4f / 16f, 1).color(255, 255, 255, 255)
					.uv(u0 + uUnit * 4, v1).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.NORTH.getNormal().getX(), Direction.NORTH.getNormal().getY(),
							Direction.NORTH.getNormal().getZ());
			...
			vertexconsumer.vertex(matrix4f, -1, 0, 0).color(255, 255, 255, 255).uv(u0, v0)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.NORTH.getNormal().getX(), Direction.SOUTH.getNormal().getY(),
							Direction.NORTH.getNormal().getZ());
			...
			vertexconsumer.vertex(matrix4f, -1 + 2f / 16f, 0, 0).color(255, 255, 255, 255)
					.uv(u0, v0 + uUnit * 2).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.WEST.getNormal().getX(), Direction.WEST.getNormal().getY(),
			...
			vertexconsumer.vertex(matrix4f, -1, 0, 1).color(255, 255, 255, 255).uv(u0, v0)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.EAST.getNormal().getX(), Direction.EAST.getNormal().getY(),
							Direction.EAST.getNormal().getZ());
			vertexconsumer.vertex(matrix4f, -1 + 2f / 16f, 0, 1).color(255, 255, 255, 255)
					.uv(u0, v0 + uUnit * 2).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmap)
					.normal(Direction.EAST.getNormal().getX(), Direction.EAST.getNormal().getY(),
							Direction.EAST.getNormal().getZ());
