...
public class EDRegistries {
	private static final RegistrySetBuilder BUILDER = new RegistrySetBuilder()
			.add(Registries.CONFIGURED_FEATURE, bootstrap -> {
				...
				HolderGetter<ConfiguredWorldCarver<?>> worldCarvers = bootstrap.lookup(Registries.CONFIGURED_CARVER);

				bootstrap.register(ExtraDelightWorldGen.CORNFIELD_BIOME, new Biome.BiomeBuilder()
						.downfall(0.4F)
//						.generationSettings(BiomeGenerationSettings.Builder.addFeature(Map.of(),
...
//										HolderSetUtil.empty(), HolderSetUtil.empty(), HolderSetUtil.empty(),
//										HolderSetUtil.empty(), HolderSetUtil.empty(), HolderSetUtil.empty(), HolderSetUtil.empty(),
//										HolderSet.direct(
//												placedFeatures.getOrThrow(ExtraDelightWorldGen.PLACED_CORN_MAZE)))))
								.generationSettings(new BiomeGenerationSettings.Builder(placedFeatures,worldCarvers)
										.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION,placedFeatures.getOrThrow(ExtraDelightWorldGen.PLACED_CORN_RAIL))
										.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION,placedFeatures.getOrThrow(ExtraDelightWorldGen.PLACED_CORN_MAZE))
										.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, placedFeatures.getOrThrow(VegetationPlacements.PATCH_PUMPKIN))
										.build())
