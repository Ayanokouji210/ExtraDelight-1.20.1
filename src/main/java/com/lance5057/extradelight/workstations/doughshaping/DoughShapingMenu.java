...
public class DoughShapingMenu extends AbstractContainerMenu {
	...
	/**
	 * The {@linkplain ItemStack} set in the input slot by
	 * the player.
	 */
...
//					createRecipeInput(pContainer), this.level);
			this.recipes = this.level.getRecipeManager()
					.getRecipesFor(ExtraDelightRecipes.DOUGH_SHAPING.get(), pContainer, this.level);
		}
	...
	void setupResultSlot() {
		if (!this.recipes.isEmpty() && this.isValidRecipeIndex(this.selectedRecipeIndex.get())) {
			DoughShapingRecipe recipeholder = this.recipes.get(this.selectedRecipeIndex.get());
//			ItemStack itemstack = recipeholder.assemble(createRecipeInput(this.container),this.level.registryAccess());
		...
		if (slot != null && slot.hasItem()) {
			...
			if (pIndex == 1) {
				...
				if (!this.moveItemStackTo(itemstack1, 2, 38, false)) {
					...
					new SimpleContainer(itemstack1),this.level).isPresent()){
